...
class ExamCard(models.Model):
    ...
    otp_code = models.CharField(max_length=12, blank=True, null=True)
...
class StudentAttendance(models.Model):
    ...
    confirmed_by = models.ForeignKey(Lecturer, on_delete=models.CASCADE, null=True, blank=True)
...
# Ensure that 80% attendance is required for exam card generation
def can_generate_exam_card(student, course):
    total_classes = StudentAttendance.objects.filter(student=student, course=course).count()
    attended_classes = StudentAttendance.objects.filter(student=student, course=course, attended=True).count()
    return attended_classes / total_classes >= 0.8 if total_classes > 0 else False